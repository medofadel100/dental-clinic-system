        ...
        {doctors?.map(doc => (
          <div key={doc.id} className={styles.detailsCard}>
            <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "1.5rem", color: "var(--primary)" }}>
              <User size={24} />
              <h2 style={{ fontSize: "1.25rem", color: "var(--text-primary)" }}>د. {doc.full_name}</h2>
            </div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "right" }}>
              ...
              <tbody>
                {DAYS_OF_WEEK.map(day => {
                  const currentSchedule = schedules?.find(s => s.doctor_id === doc.id && s.day_of_week === day.id);
                  return (
                    <tr key={day.id} style={{ borderBottom: "1px solid var(--border)" }}>
                      <td style={{ padding: "1rem 0.5rem" }}>{day.name}</td>
                      <td style={{ padding: "1rem 0.5rem" }}>
                        <form id={`form-${doc.id}-${day.id}`} action={saveSchedule} style={{ display: "flex", gap: "1rem", alignItems: "center" }}>
                          <input type="hidden" name="doctor_id" value={doc.id} />
                          <input type="hidden" name="day_of_week" value={day.id} />
                          <input type="time" name="start_time" defaultValue={currentSchedule?.start_time?.slice(0, 5) || "10:00"} 
                            style={{ padding: "0.5rem", borderRadius: "0.25rem", border: "1px solid var(--border)" }} />
                        </form>
                      </td>
                      <td style={{ padding: "1rem 0.5rem" }}>
                        <input type="time" name="end_time" form={`form-${doc.id}-${day.id}`} defaultValue={currentSchedule?.end_time?.slice(0, 5) || "22:00"} 
                          style={{ padding: "0.5rem", borderRadius: "0.25rem", border: "1px solid var(--border)" }} />
                      </td>
                      <td style={{ padding: "1rem 0.5rem" }}>
                        <select name="is_active" form={`form-${doc.id}-${day.id}`} defaultValue={currentSchedule?.is_active === false ? "false" : "true"}
                          style={{ padding: "0.5rem", borderRadius: "0.25rem", border: "1px solid var(--border)" }}>
                      ...
                      <td style={{ padding: "1rem 0.5rem" }}>
                        <button type="submit" form={`form-${doc.id}-${day.id}`} className={styles.primaryBtn} style={{ padding: "0.5rem 1rem", fontSize: "0.875rem" }}>
                          <Save size={16} /> حفظ
                        </button>
                      </td>
                    </tr>
