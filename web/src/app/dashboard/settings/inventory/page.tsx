...
export default async function InventorySettingsPage() {
  ...
  async function updateMinStock(formData: FormData) {
    ...
    if (itemId && minStock >= 0) {
      await supabase
        .from("inventory")
        .update({ minimum_stock_level: minStock })
        .eq("id", itemId);
      ...
      <div className={styles.detailsCard} style={{ marginTop: "2rem" }}>
        ...
        >
          ...
          <h2 style={{ fontSize: "1.25rem", color: "var(--text-primary)" }}>
            تحديد الحد الأدنى للتنبيه لكل صنف
          </h2>
        </div>
        <p
          style={{
        ...
        >
          قم بتحديث الحد الأدنى لكل منتج. بمجرد أن تصل كمية المنتج إلى هذا الحد،
          سيظهر تنبيه أحمر في الصفحة الرئيسية.
        </p>

        {!inventory || inventory.length === 0 ? (
          <p className={styles.placeholderText}>
            لا يوجد أصناف مسجلة في المخزن حالياً.
          </p>
        ) : (
          <div
