...
export default async function AppointmentsPage({ searchParams }: { searchParams: Promise<{ doctorId?: string, search?: string }> }) {
  ...
  if (filterSearch) {
    ...
    if (matchedPatients && matchedPatients.length > 0) {
      query = query.in('patient_id', matchedPatients.map(p => p.id));
    } else {
  ...
  const events = appointments?.map((appt: any) => {
    ...
    const doctorName = appt.profiles ? `د. ${appt.profiles.full_name}` : 'أي طبيب';
    ...
    <div className={styles.container}>
      <div className={styles.header}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
          ...
          <form style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', backgroundColor: 'var(--bg-surface)', padding: '0.5rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--border)' }}>
            ...
            <input 
              ...
              style={{ border: 'none', backgroundColor: 'transparent', color: 'var(--text-primary)', outline: 'none', fontSize: '0.875rem', width: '150px' }} 
            />
            <div style={{ width: '1px', height: '20px', backgroundColor: 'var(--border)' }}></div>
            <select 
              name="doctorId" 
              defaultValue={filterDoctorId || ""} 
              style={{ border: 'none', backgroundColor: 'transparent', color: 'var(--text-primary)', outline: 'none', fontSize: '0.875rem' }}
            >
            ...
            <button type="submit" style={{ padding: '0.25rem 0.5rem', backgroundColor: 'var(--bg-main)', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', color: 'var(--text-primary)', cursor: 'pointer', fontSize: '0.75rem' }}>
              بحث وتصفية
            </button>
          </form>
        </div>

        <Link href="/dashboard/appointments/new" className={styles.primaryBtn} style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <Plus size={18} />
