...
export default async function ReportsPage({
  ...
  if (endDate)
    expensesQuery = expensesQuery.lte("created_at", `${endDate}T23:59:59.999Z`);
  ...
  let salariesQuery = supabase
    ...
    .eq("category", "Salary");
  if (startDate)
    salariesQuery = salariesQuery.gte(
      "created_at",
      `${startDate}T00:00:00.000Z`,
    );
  if (endDate)
    salariesQuery = salariesQuery.lte("created_at", `${endDate}T23:59:59.999Z`);
  ...
  let debtsQuery = supabase
    ...
    .eq("is_paid", true);
  if (startDate)
    debtsQuery = debtsQuery.gte("created_at", `${startDate}T00:00:00.000Z`);
  if (endDate)
    debtsQuery = debtsQuery.lte("created_at", `${endDate}T23:59:59.999Z`);
  ...
  const totalDebtsPaid =
    clinicDebts?.reduce((sum, debt) => sum + Number(debt.amount || 0), 0) || 0;
      ...
      >
        ...
        >
          ...
          <div>
            ...
            >
              {(totalSalaries + totalDebtsPaid).toLocaleString()} ج.م
            </div>
          </div>
        </div>
        ...
        <div
          style={{
            ...
            borderRadius: "var(--radius-lg)",
            border: "2px solid",
            borderColor: netProfit >= 0 ? "var(--success)" : "var(--error)",
            display: "flex",
            alignItems: "center",
            gap: "1.5rem",
          }}
