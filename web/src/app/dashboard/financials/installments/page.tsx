...
export default async function InstallmentsPage({
  ...
  return (
    <div style={{ padding: "2rem", maxWidth: "1200px", margin: "0 auto" }}>
      ...
      >
        <Link
          href={`?tab=patients${startDate ? `&start=${startDate}` : ""}${endDate ? `&end=${endDate}` : ""}`}
          style={{
            ...
            textDecoration: "none",
            backgroundColor:
              activeTab === "patients" ? "var(--primary)" : "var(--bg-surface)",
            color: activeTab === "patients" ? "white" : "var(--text-secondary)",
            border:
              activeTab === "patients" ? "none" : "1px solid var(--border)",
          }}
        >
          أقساط المرضى (للعيادة)
        </Link>
        <Link
          href={`?tab=clinic${startDate ? `&start=${startDate}` : ""}${endDate ? `&end=${endDate}` : ""}`}
          style={{
            padding: "0.75rem 1.5rem",
            borderRadius: "var(--radius-md)",
            fontWeight: 600,
            textDecoration: "none",
            backgroundColor:
              activeTab === "clinic" ? "var(--primary)" : "var(--bg-surface)",
          ...
          {patientInstallments?.length === 0 ? (
            ...
            >
              <p style={{ color: "var(--text-secondary)" }}>
                لا توجد أي دفعات مستحقة حالياً على المرضى بالفترة المحددة.
              </p>
            </div>
          ) : (
            patientInstallments?.map((inst: any) => {
              const remaining =
                Number(inst.amount_due) - Number(inst.amount_paid);
              return (
                <div
                  key={inst.id}
                  style={{
                ...
                >
                  ...
                  >
                    ...
                    >
                      تاريخ الجلسة:{" "}
                      {new Date(inst.created_at).toLocaleDateString("ar-EG")}
                    </div>
                  </div>
                  <div
                    style={{
        ...
        <div>
          ...
          >
            <h3 style={{ marginBottom: "1rem" }}>
              إضافة قسط / مديونية جديدة للعيادة
            </h3>
            <form
              action={addClinicDebt}
              style={{
                display: "flex",
                gap: "1rem",
                alignItems: "flex-end",
          ...
          >
            {debtsError ? (
              ...
              >
                يرجى تنفيذ أمر إنشاء جدول (clinic_debts) في قاعدة البيانات أولاً
                لتفعيل هذه الميزة.
              </div>
              ...
              >
                <p style={{ color: "var(--text-secondary)" }}>
                  لا توجد مديونيات أو أقساط مسجلة على العيادة بالفترة المحددة.
                </p>
              </div>
            ) : (
                ...
                >
                  ...
                  <div
                    style={{
                      fontSize: "1.5rem",
                      fontWeight: "bold",
                      color: debt.is_paid ? "var(--success)" : "var(--error)",
                    }}
                  >
                    {debt.amount} ج.م
                  </div>
                  ...
                  {!debt.is_paid ? (
                    ...
                    >
                      <div>
                        تم الدفع: {getPaymentMethodName(debt.payment_method)}
                      </div>
                      {debt.receipt_number && (
                        <div>إيصال: {debt.receipt_number}</div>
                      )}
                    </div>
                  )}
                </div>
