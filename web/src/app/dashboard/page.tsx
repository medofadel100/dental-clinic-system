...
export default async function DashboardOverview() {
  ...
  const { data: pendingInvoices } = await supabase
    .from("invoices")
    .select(
      "id, amount_due, amount_paid, status, created_at, patients(full_name)",
    )
  ...
  tasks.sort(
    (a, b) =>
      Math.abs(a.date.getTime() - Date.now()) -
      Math.abs(b.date.getTime() - Date.now()),
  );
            ...
            >
              {lowStockItems.map((item, idx) => (
                <li key={idx} style={{ marginBottom: "0.25rem" }}>
                  <strong>{item.item_name}</strong> - الكمية الحالية:{" "}
                  {item.quantity} (الحد الأدنى: {item.minimum_stock_level})
                </li>
              ))}
            </ul>
          </div>
      ...
      <div className={styles.statsGrid}>
        ...
        >
          <div
            className={styles.statCard}
            style={{
              border: lowStockCount > 0 ? "1px solid var(--error)" : "none",
              cursor: "pointer",
              transition: "transform 0.2s",
              height: "100%",
            }}
      ...
      <div className={styles.chartsGrid}>
        <div className={styles.chartCard}>
          ...
          {todayAppts && todayAppts.length > 0 ? (
              ...
              {todayAppts.map((appt) => (
                ...
                >
                  ...
                  >
                    ...
                    >
                      {appt.status === "Scheduled" ? "مجدول" : appt.status}
                    </span>
                  </div>
            ...
            <p className={styles.placeholderText} style={{ marginTop: "1rem" }}>
        ...
        <div className={styles.chartCard}>
          ...
          {topTasks.length > 0 ? (
              ...
              {topTasks.map((task) => (
                <li
                  ...
                  style={{
                    ...
                    borderRadius: "var(--radius-md)",
                    border: `1px solid ${task.type === "invoice" ? "rgba(239, 68, 68, 0.2)" : "rgba(59, 130, 246, 0.2)"}`,
            ...
            <p className={styles.placeholderText} style={{ marginTop: "1rem" }}>
              لا توجد مهام معلقة! النظام محدث بالكامل.
            </p>
          )}
        </div>
      </div>
