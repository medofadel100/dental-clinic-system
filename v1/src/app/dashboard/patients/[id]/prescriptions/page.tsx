...
import { sendPrescriptionWhatsApp, requestXRayWhatsApp, getPatientPrescriptions } from "./actions";
...
export default function PrescriptionsPage({
  ...
  const handleRequestXRay = async () => {
    const xrayType = prompt("ما هو نوع الأشعة المطلوب؟ (مثال: بانوراما، أشعة مقطعية)");
    if (!xrayType) return;
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <button 
          ...
          style={{
            ...
            borderRadius: "var(--radius-md)", cursor: xrayLoading ? "not-allowed" : "pointer", 
            fontFamily: "inherit", fontWeight: 600, opacity: xrayLoading ? 0.7 : 1
          }}
        >
          <ImageIcon size={18} />
          <span>{xrayLoading ? "جاري الإرسال..." : "طلب أشعة عبر الواتساب"}</span>
        </button>
      </div>
        ...
        <div style={{
          ...
          {success && (
            <div style={{
              ...
              color: "var(--success)", borderRadius: "var(--radius-md)", marginBottom: "1.5rem",
          ...
          {pastPrescriptions.length === 0 ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
              {pastPrescriptions.map((rx) => (
                <div key={rx.id} style={{
                  padding: "1rem", backgroundColor: "var(--bg-main)",
                  borderRadius: "var(--radius-md)", border: "1px solid var(--border)",
                  display: "flex", flexDirection: "column", gap: "0.5rem"
                }}>
                  <div style={{ fontSize: "0.75rem", color: "var(--text-muted)", marginBottom: "0.25rem" }}>
                    {new Date(rx.created_at).toLocaleDateString('ar-EG', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                  </div>
                  <div style={{ whiteSpace: "pre-line", fontSize: "0.875rem", fontWeight: 500 }}>
                    {rx.medications}
                  </div>
                  {rx.notes && (
                    <div style={{ fontSize: "0.875rem", color: "var(--text-secondary)", marginTop: "0.5rem", borderTop: "1px dashed var(--border)", paddingTop: "0.5rem" }}>
                      ملاحظات: {rx.notes}
                    </div>
                  )}
                </div>
